...
의 코드가 나온다. 일단 두번째 글자부터 보자면,

    0x08048454      movzx eax, byte [0x804a021]
    0x0804845b      cmp al, 0x31
    0x0804845d      je 0x8048469
    
주소 `0x0804a021`에 들어가 있는 1바이트 문자가 `0x31` (`1`)인지 체크한다. 코드에선 이 부분이 문자를 체크하는데 있어 첫번째 순서로 나오는데, 이 문자'만' 숫자라서 그런 게 아닐까 싶다. 글자가 각 위치에서 확인하려는 것과 일치하는 경우엔 다음 글자를 확인하는 과정으로 넘어가지만, 만약 아니라면... 항상,
...
그리고 나머지 문자들은... 맨 마지막 글자의 위치가 5번째 글자, 그러니까 `0x0804a024` 주소란 걸 알 수 있는데, 이건...
...
...으로 XOR 연산을 취한 다음 바뀐 글자를 원래 글자가 있는 위치에 넣어두고, 이후...

    0x080484**      movzx eax, byte [0x804a02*]
    0x080484**      cmp al, 0x**
    0x080484**      je 0x80484**
    
... 다시 가져와서 맨 밑 1바이트 부분이 특정 코드와 일치하는지를 본다. 맞으면 다른 글자를 체크하는 부분으로 넘어가고, 틀리면 eax를 0으로 바꾸는 그 코드 파트가 실행된다.
